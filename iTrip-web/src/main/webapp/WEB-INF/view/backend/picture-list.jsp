...
<body>
	...
	<div class="page-container">
		<form action="getBackendPicture_list" method="post">
			<div class="text-c">
				日期范围： <input type="text" name="creationDate" value="${creationDate}"
				...
				<input type="text" name="linkUserName" id="" placeholder=" 预订人姓名"
					style="width: 250px" class="input-text" value="${linkUserName}">
				<input name="" id="" class="btn btn-success" type="submit" value="搜索订单"/>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> <a class=""
				onclick="picture_add('添加订单','getOrders_add')" href="javascript:;">
					<i class="Hui-iconfont"></i> 
			</a></span> <span class="r">共有数据：<strong>${count}</strong> 条
			</span>
		</div>
		<div class="mt-20">
			<table
				...
				<tbody>
					<c:forEach items="${listOrders}" var="orders">
						<tr class="text-c">
							...
							<td>${orders.orderNo}<br> ${orders.hotelName}
							</td>
							<td>${orders.roomName }</td>
							<td>${orders.count }</td>
							<td class="text-l">入住：<fmt:formatDate value="${orders.checkInDate }" pattern="yyyy-MM-dd HH:mm"/> 
							<br />
								离店：<fmt:formatDate value="${orders.checkOutDate }" pattern="yyyy-MM-dd HH:mm"/> 
							</td>
							<td class="text-c">${orders.payAmount }</td>
							<td><fmt:formatDate value="${orders.creationDate }" pattern="yyyy-MM-dd HH:mm"/> </td>
							<td>${orders.linkUserName}</td>
							<td>${orders.noticePhone}</td>
							<td class="td-status"><span
								class="label label-success radius"> <c:if
										test="${orders.orderStatus ==0 }">待支付</c:if> <c:if
										test="${orders.orderStatus ==1 }">已取消 </c:if> <c:if
										test="${orders.orderStatus ==2 }">支付成功 </c:if> <c:if
										test="${orders.orderStatus ==3 }">已消费</c:if></span></td>

							<td class="td-manage"><a style="text-decoration: none"
								class="ml-5"
								onClick="picture_edit('订单编辑','getOrders_add?id=${orders.id}','10001')"
